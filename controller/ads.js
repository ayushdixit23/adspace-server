...
const createAd = async (req, res) => {
  try {
    ...
    const newAd = new ads({
      ...
      category: parsedData.communityTags,
      cta: parsedData.actionAndUrl?.action || null,
      ctalink: parsedData.actionAndUrl.url,
      content: parsedData.file
        ? [
            {
              extension: parsedData.file.filename,
              name: parsedData.file.filename,
            },
          ]
        : [],
      type: parsedData.type,
      tags: parsedData.interestTags || [],
...
const getAdsStats = async (req, res) => {
  try {
    const { startDate, endDate, campaignOptions } = req.query;
    ...
    console.log("Active Campaign Options:", activeCampaignOptions);
